    ...
    <div style={INNER}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <h2 style={{ fontSize: '18px', fontWeight: 700, color: '#c9d1d9', letterSpacing: '-0.3px' }}>Regras de Alerta</h2>
        </div>
        <button onClick={() => { setShowForm(s => !s); setError(''); }} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 18px', background: '#ca8a04', color: '#fff', border: 'none', borderRadius: '6px', fontFamily: 'inherit', fontSize: '13px', fontWeight: 700, cursor: 'pointer' }}>
      ...
      {showForm && (
        <div style={{ ...CARD, marginBottom: '20px', borderColor: 'rgba(234,179,8,0.25)' }}>
          <h3 style={{ fontSize: '14px', fontWeight: 600, color: '#c9d1d9', marginBottom: '18px' }}>Nova Regra de Alerta</h3>
          ...
          <form onSubmit={handleCreate}>
            ...
            <p style={{ fontSize: '12px', color: '#484f58', marginBottom: '18px' }}>
              Dispara quando houver ≥ <strong style={{ color: '#c9d1d9' }}>{form.threshold}</strong> eventos
              {form.severity && <> com severidade <strong style={{ color: SEV_COLOR[form.severity] }}>≥ {SEV_LABEL[form.severity] || form.severity}</strong></>}
              {form.source && <> · fonte <strong style={{ color: '#eab308' }}>{form.source}</strong></>}
              {form.keyword && <> · mensagem contém <strong style={{ color: '#a371f7' }}>"{form.keyword}"</strong></>}
              {' '}nos últimos <strong style={{ color: '#c9d1d9' }}>{form.window_minutes}</strong> min.
            </p>
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
              <button type="button" onClick={() => setShowForm(false)} style={{ padding: '8px 18px', background: 'transparent', border: '1px solid rgba(48,54,61,0.8)', color: '#8b949e', borderRadius: '6px', fontFamily: 'inherit', fontSize: '13px', cursor: 'pointer' }}>Cancelar</button>
      ...
      <div style={CARD}>
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {rules.map(rule => (
              <div key={rule.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', background: 'rgba(0,0,0,0.2)', borderRadius: '8px', border: `1px solid ${rule.active ? 'rgba(88,166,255,0.15)' : 'rgba(48,54,61,0.4)'}` }}>
                <div>
                  <p style={{ margin: 0, fontWeight: 600, color: rule.active ? '#c9d1d9' : '#484f58', fontSize: '14px' }}>{rule.name}</p>
                  <p style={{ margin: '4px 0 0', fontSize: '12px', color: '#8b949e' }}>
                    ≥ {rule.threshold} eventos
                    {rule.severity && <> · <span style={{ color: SEV_COLOR[rule.severity] || '#8b949e' }}>≥ {rule.severity}</span></>}
                    {rule.source && <> · <span style={{ color: '#eab308' }}>{rule.source}</span></>}
                    {rule.keyword && <> · <span style={{ color: '#a371f7' }}>"{rule.keyword}"</span></>}
                    {' '}· {rule.window_minutes} min
                  </p>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <button onClick={() => handleToggle(rule)} title={rule.active ? 'Desativar' : 'Ativar'} style={{ padding: '5px 8px', background: rule.active ? 'rgba(63,185,80,0.1)' : 'rgba(139,148,158,0.1)', color: rule.active ? '#3fb950' : '#8b949e', border: 'none', borderRadius: '5px', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
                    {rule.active ? <ToggleRight size={16} /> : <ToggleLeft size={16} />}
                  </button>
                  <button onClick={() => handleDelete(rule.id, rule.name)} title="Remover" style={{ padding: '5px 8px', background: 'rgba(248,81,73,0.1)', color: '#f85149', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>
                    <Trash2 size={13} />
                  </button>
                </div>
              </div>
