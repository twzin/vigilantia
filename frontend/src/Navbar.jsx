...
import { ShieldAlert, LayoutDashboard, Search, LogOut, Users, Bell, BellRing } from 'lucide-react';
...
export default function Navbar({ role }) {
  ...
  return (
    <header style={{
      ...
      padding: '0 24px', height: '56px', background: '#0d0d1a',
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <span style={{ fontWeight: 700, fontSize: '15px', letterSpacing: '-0.3px', color: '#e8eaf0' }}>Vigilantia</span>
        </div>
        {role && (
          <span style={{ fontSize: '10px', fontWeight: 700, padding: '2px 9px', borderRadius: '999px', textTransform: 'uppercase', letterSpacing: '0.8px', color: s.color, background: s.bg }}>
            {s.label}
          </span>
        )}
      </div>
      ...
      <nav style={{ display: 'flex', alignItems: 'center', gap: '2px' }}>
        ...
        {navBtn('/logs',      'Logs',      Search)}
        {navBtn('/alertas',   'Alertas',   BellRing)}

        {role === 'admin' && (
          <>
            <div style={{ width: '1px', height: '20px', background: 'rgba(255,255,255,0.08)', margin: '0 6px' }} />
            {navBtn('/admin/usuarios', 'Usuários', Users)}
            {navBtn('/admin/alertas',  'Regras',   Bell)}
          </>
        )}
